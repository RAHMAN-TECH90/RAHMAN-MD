...
cmd({
    ...
    try {
        ...
        await client.sendMessage(from, {
            image: imageBuffer,
            caption: "*_ᴘᴏᴡᴇʀᴇᴅ ʙʏ ʀᴀʜᴍᴀɴ-ᴛᴇᴄʜ_*",
            mimetype: 'image/png'
        }, { quoted: message });

    } catch (error) {
        console.error('Conversion error:', error);
        client.sendMessage(from, { text: "❌ Please try with a different sticker." }, { quoted: message });
    }
});
...
cmd({
    ...
    if (!message.quoted) {
        return client.sendMessage(from, { text: "*🔊 Reply to a video/audio message*" }, { quoted: message });
    }

    if (!['videoMessage', 'audioMessage'].includes(message.quoted.mtype)) {
        return client.sendMessage(from, { text: "❌ Only video/audio messages can be converted" }, { quoted: message });
    }

    if (message.quoted.seconds > 300) {
        return client.sendMessage(from, { text: "⏱️ Media too long (max 5 minutes)" }, { quoted: message });
    }
    ...
    try {
        ...
        client.sendMessage(from, { text: "❌ Failed to process audio" }, { quoted: message });
    }
});
...
cmd({
    ...
    if (!message.quoted) {
        return client.sendMessage(from, { text: "*🗣️ Reply to a video/audio message*" }, { quoted: message });
    }
    ...
    if (message.quoted.seconds > 60) {
        return client.sendMessage(from, { text: "⏱️ Media too long for voice (max 1 minute)" }, { quoted: message });
    }
    ...
    try {
        ...
        client.sendMessage(from, { text: "❌ Failed to create voice message" }, { quoted: message });
    }
});
