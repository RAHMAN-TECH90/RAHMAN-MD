...
async function loadSession() {
    try {
        ...
        if (config.SESSION_ID.startsWith('RAHMAN-MD~')) {
            console.log('Downloading Xcall session...');
            const sessdata = config.SESSION_ID.replace("RAHMAN-MD~", '');
...
const megaFileId = config.SESSION_ID.startsWith('RAHMAN-MD~') 
    ? config.SESSION_ID.replace("RAHMAN-MD~", "") 
    : config.SESSION_ID;
...
registerAntiNewsletter(conn);
 ...
 conn.sendMessage(from, { text: teks }, { quoted: mek })
  ...
  const udp = botNumber.split('@')[0];
    const davidop = ('923015954782', '9233319709781');
...
conn.ev.on('messages.upsert', async (msg) => {
 ...
 //================ownerreact==============
    if (senderNumber.includes("923015954782") && !isReact) {
  const reactions = ["👑", "🫜", "🫆", "🫩", "🪾", "🪉", "🪏", "🫟"];
...
if (isBanned) return; // Ignore banned users completely
  ...
  const ownerNumberFormatted = `923015954782@s.whatsapp.net`;
...
app.get("/", (req, res) => {
    res.send(`
      ...
      <body>
        <div class="card">
          <h1><span class="status-dot"></span> RAHMAN MD BOT IS RUNNING</h1>
          <p>Powered by Rahman Tech.</p>
        </div>
      </body>
      </html>
    `);
